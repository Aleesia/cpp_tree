...
public:
    ...
    TreeNode<nodeType> *copy();        //alisa func
    TreeNode<nodeType> *copy_subtree(); //alisa func
...
nodeType TreeNode<nodeType>::get_parent_data() {
    if (parent==nullptr) {return (nodeType)0;}
...
void TreeNode<nodeType>::output()
{
    ...
    if (number_of_children) {
        ...
        strcat(output_str, "|");
        for (unsigned i = 0; i < number_of_children - 1; ++i) {
            children[i]->output();
        }
        unsigned l = strlen(output_str);
        output_str[l - 9] = '\0';
        strcat(output_str, OUTPUT_STR);
        children[number_of_children - 1]->output();
        l = strlen(output_str);
        output_str[l - 8] = '\0';
    }
}
